...
int device_start( const char* name, int xsize, int ysize, window_manager* wm )
{
    ...
    xsize = sprofile_get_int_value( KEY_SCREENX, xsize, 0 );
    ysize = sprofile_get_int_value( KEY_SCREENY, ysize, 0 );
    ...
    wm->delete_message = XInternAtom( wm->dpy, "WM_DELETE_WINDOW", false );
    XSetWMProtocols( wm->dpy, wm->win, &wm->delete_message, 1 );

    //Selection:
    wm->sel_atom_clipboard = XInternAtom( wm->dpy, "CLIPBOARD", 0 );
    wm->sel_atom_xsel_data = XInternAtom( wm->dpy, "XSEL_DATA", 0 );
    wm->sel_atom_targets = XInternAtom( wm->dpy, "TARGETS", 0 );
    wm->sel_atom_text = XInternAtom( wm->dpy, "TEXT", 0 );
    wm->sel_atom_utf8_string = XInternAtom( wm->dpy, "UTF8_STRING", 1 );
    if( wm->sel_atom_utf8_string == None ) wm->sel_atom_utf8_string = XA_STRING;

    if( sprofile_get_int_value( KEY_NOCURSOR, -1, 0 ) != -1 )
    {
	//Hide mouse pointer:
	Cursor cursor;
	Pixmap blank;
...
void device_event_handler( window_manager* wm )
{
        ...
        switch( evt.type ) 
	{
	    ...
	    case ClientMessage:
		if( evt.xclient.data.l[ 0 ] == (long)wm->delete_message )
		{
		    send_event( 0, EVT_QUIT, wm );
		}
		break;
		...
		if( key )
		{
		    if( evt.type == KeyPress )
		    {
			send_event( 0, EVT_BUTTONDOWN, wm->mods, 0, 0, key, 0, 1024, wm );
		    }
		    else
		    {
			send_event( 0, EVT_BUTTONUP, wm->mods, 0, 0, key, 0, 1024, wm );
		    }
		}
		break;
	    }
	    ...
	    {
		if( evt.xselectionrequest.selection != wm->sel_atom_clipboard ) break;
		...
		while( 1 )
		{
		    if( xsr->target == wm->sel_atom_targets )
		    {
			XChangeProperty( e.display, e.requestor, e.property, XA_ATOM, 32, PropModeReplace, (unsigned char*)&wm->sel_atom_utf8_string, 1 );
			break;
		    }
		    Atom target = xsr->target;
		    if( target == XA_STRING || target == wm->sel_atom_text || xsr->target == wm->sel_atom_utf8_string )
		    {

			fsize = sfs_get_file_size( "3:/sundog_clipboard" );
			if( fsize )
			{
			    fdata = (char*)smem_new( fsize + 1 );
			    if( fdata )
	    		    {
	    	        	sfs_file f = sfs_open( "3:/sundog_clipboard", "rb" );
	        		if( f )
	    		        {
			...
			if( fdata )
			{
			    if( target == wm->sel_atom_text ) target = XA_STRING;
			    XChangeProperty( e.display, e.requestor, e.property, target, 8, PropModeReplace, (const unsigned char*)fdata, fsize );
			    break;
			}
		    }
		    e.property = None;
		...
		XSendEvent( wm->dpy, e.requestor, 0, 0, (XEvent*)&e );
	    ...
	    {
		if( evt.xselection.selection != wm->sel_atom_clipboard ) break;
		if( evt.xselection.property )
		{
		    ...
		    if( size && data )
		    {
			void* dest = smem_new( size );
			if( dest )
			{
			    smem_copy( dest, data, size );
			    smbox_msg* msg = smbox_new_msg();
			    if( msg )
			    {
				msg->id = g_msg_id_x11_paste;
				msg->data = dest;
				msg->lifetime_s = 20;
				smbox_add( wm->sd->mb, msg );
			    }
	...
	{ //No more X11 events
	    if( wm->events_count == 0 ) //And no WM events
	    {
        	int pause_ms = ( wm->ticks_per_frame - ( cur_t - wm->prev_frame_time ) ) * 1000 / stime_ticks_per_second_hires();
