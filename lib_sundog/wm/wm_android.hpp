...
void device_event_handler( window_manager* wm )
{
	...
	for( int i = 0; i < 5; i++ ) //delayed surface resize?
	{
		...
		android_sundog_screen_redraw(); //eglSwapBuffers can resize surface prior to copying its pixels to the native window
	    }
	    stime_sleep( 10 );
	}
...
{
    /*int rv = -1;
    char* name_in = sfs_make_filename( wm->vcap_in_name, true );
