/*
    ...
    Copyright (C) 2010 - 2022 Alexander Zolotov <nightradio@gmail.com>
    WarmPlace.ru
*/
...
static void colortheme_set_ctls( colortheme_data* data )
{
	...
	COLOR col = colortheme_get_color( wm->color_theme, x, wm );
	    ...
	    ckey[ 2 ] = '0' + x;
	    ckey[ 3 ] = 0;
	    char* tv; tv = sprofile_get_str_value( (const char*)ckey, 0, 0 ); if( tv ) col = get_color_from_string( tv );
	}
	for( int y = 0; y < 3; y++ )
	{
    	    int v = 0;
...
static void colortheme_save_ctls( colortheme_data* data )
{
	...
	get_string_from_color( col_str, sizeof( col_str ), col );
        ...
        sprofile_set_str_value( (const char*)ckey, col_str, 0 ); 
...
static int colortheme_ok_handler( void* user_data, WINDOWPTR win, window_manager* wm )
{
    ...
    sprofile_set_int_value( KEY_COLOR_THEME, wm->color_theme, 0 );
    if( data->user_theme )
    {
	sprofile_set_int_value( "theme", 1, 0 );
	colortheme_save_ctls( data );
    }
    else
    {
	sprofile_remove_key( "theme", 0 );
    }
    sprofile_save( 0 );
...
int colortheme_handler( sundog_event* evt, window_manager* wm )
{
	...
	case EVT_AFTERCREATE:
		...
		data->user_theme_changed = false;
	        if( sprofile_get_int_value( "theme", -1, 0 ) != -1 ) data->user_theme = true;
