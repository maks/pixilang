...
void empty_device_event_handler( window_manager* wm )
{
    ...
    if( pause )
        stime_sleep( pause );
    ticks_hr_t cur_t = stime_ticks_hires();
...
int win_init( const char* name, int xsize, int ysize, uint flags, sundog_engine* sd )
{
    ...
    name = (const char*)sprofile_get_str_value( KEY_WINDOWNAME, name, 0 );
    int angle = sprofile_get_int_value( KEY_ROTATE, 0, 0 );
    if( angle > 0 ) flags |= ( ( angle / 90 ) & 3 ) << WIN_INIT_FLAGOFF_ANGLE;
    int zoom = sprofile_get_int_value( KEY_ZOOM, 0, 0 );
    if( zoom > 0 ) flags |= ( zoom & 7 ) << WIN_INIT_FLAGOFF_ZOOM;
    if( sprofile_get_int_value( KEY_FULLSCREEN, -1, 0 ) != -1 ) flags |= WIN_INIT_FLAG_FULLSCREEN;
    if( sprofile_get_int_value( KEY_MAXIMIZED, -1, 0 ) != -1 ) flags |= WIN_INIT_FLAG_MAXIMIZED;
    if( sprofile_get_int_value( KEY_VSYNC, -1, 0 ) != -1 ) flags |= WIN_INIT_FLAG_VSYNC;
#ifdef FRAMEBUFFER
    flags |= WIN_INIT_FLAG_FRAMEBUFFER;
#endif
    int fb = sprofile_get_int_value( KEY_FRAMEBUFFER, -1, 0 );
    if( fb == 1 ) flags |= WIN_INIT_FLAG_FRAMEBUFFER;
    if( fb == 0 ) flags &= ~WIN_INIT_FLAG_FRAMEBUFFER;
    flags |= WIN_INIT_FLAG_OPTIMIZE_MOVE_EVENTS;
    if( sprofile_get_int_value( KEY_NOWINDOW, -1, 0 ) != -1 ) flags |= WIN_INIT_FLAG_NOWINDOW;
    ...
    #endif
#endif
    wm->screen_buffer_preserved = sprofile_get_int_value( KEY_SCREENBUF_SWAP_BEHAVIOR, wm->screen_buffer_preserved, 0 );
    ...
    wm->screen_zoom = 1;
#endif
    wm->screen_ppi = sprofile_get_int_value( KEY_PPI, 0, 0 ); //Default - 0 (unknown)
    wm->screen_scale = (float)sprofile_get_int_value( KEY_SCALE, 0, 0 ) / 256.0F; //Default - 0 (unknown)
    wm->screen_font_scale = (float)sprofile_get_int_value( KEY_FONT_SCALE, 0, 0 ) / 256.0F; //Default - 0 (unknown)
...
#endif
    if( sprofile_get_int_value( KEY_SHOW_VIRT_KBD, -1, 0 ) != -1 )
	wm->show_virtual_keyboard = true;
    if( sprofile_get_int_value( KEY_HIDE_VIRT_KBD, -1, 0 ) != -1 )
	wm->show_virtual_keyboard = false;
...
#endif
    int show = sprofile_get_int_value( KEY_SHOW_ZOOM_BUTTONS, -1, 0 );
    ...
    wm->font_small = sprofile_get_int_value( KEY_FONT_SMALL, DEFAULT_FONT_SMALL, 0 );
...
device_restart:
    ...
    wm->max_fps = MAX_FPS;
#endif
    wm->max_fps = sprofile_get_int_value( KEY_MAXFPS, wm->max_fps, 0 );
...
{
    wm->custom_colors = sprofile_get_str_value( "theme", 0, 0 );
    ...
    {
	char* v;
	v = sprofile_get_str_value( "c_0", 0, 0 ); if( v ) wm->color0 = get_color_from_string( v );
	v = sprofile_get_str_value( "c_1", 0, 0 ); if( v ) wm->color1 = get_color_from_string( v );
	v = sprofile_get_str_value( "c_2", 0, 0 ); if( v ) wm->color2 = get_color_from_string( v );
	v = sprofile_get_str_value( "c_3", 0, 0 ); if( v ) wm->color3 = get_color_from_string( v );
	v = sprofile_get_str_value( "c_yellow", 0, 0 ); if( v ) wm->yellow = get_color_from_string( v );
	v = sprofile_get_str_value( "c_green", 0, 0 ); if( v ) wm->green = get_color_from_string( v );
	v = sprofile_get_str_value( "c_red", 0, 0 ); if( v ) wm->red = get_color_from_string( v );
	v = sprofile_get_str_value( "c_blue", 0, 0 ); if( v ) wm->blue = get_color_from_string( v );
    }
    ...
    {
	char* v;
	v = sprofile_get_str_value( "c_header", 0, 0 ); if( v ) wm->header_text_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_alt", 0, 0 ); if( v ) wm->alternative_text_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_dlg", 0, 0 ); if( v ) wm->dialog_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_dec", 0, 0 ); if( v ) wm->decorator_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_decbord", 0, 0 ); if( v ) wm->decorator_border = get_color_from_string( v );
	v = sprofile_get_str_value( "c_btn", 0, 0 ); if( v ) wm->button_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_pbtn", 0, 0 ); if( v ) wm->pressed_button_color = get_color_from_string( v );
	wm->pressed_button_color_opacity = sprofile_get_int_value( "c_pbtn_op", wm->pressed_button_color_opacity, 0 );
	v = sprofile_get_str_value( "c_menu", 0, 0 ); if( v ) wm->menu_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_sel", 0, 0 ); if( v ) wm->selection_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_txtback", 0, 0 ); if( v ) wm->text_background = get_color_from_string( v );
	v = sprofile_get_str_value( "c_lstback", 0, 0 ); if( v ) wm->list_background = get_color_from_string( v );
	v = sprofile_get_str_value( "c_scroll", 0, 0 ); if( v ) wm->scroll_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_scrollback", 0, 0 ); if( v ) wm->scroll_background_color = get_color_from_string( v );
	v = sprofile_get_str_value( "c_pscroll", 0, 0 ); if( v ) wm->scroll_pressed_color = get_color_from_string( v );
	//wm->scroll_pressed_color_opacity = sprofile_get_int_value( "c_pscroll_op", wm->scroll_pressed_color_opacity, 0 ); //255 will hide some details in the standard scrollbar?
    }
}
...
void win_deinit( window_manager* wm )
{
    ...
    {
	if( sprofile_get_int_value( KEY_DONT_SAVE_WINSIZE, -1, 0 ) == -1 )
	{
	    bool save = 0;
	    int w = sprofile_get_int_value( KEY_SCREENX, 0, 0 );
    	    int h = sprofile_get_int_value( KEY_SCREENY, 0, 0 );
    	    bool m = 0;
    	    if( sprofile_get_int_value( KEY_MAXIMIZED, -1, 0 ) != -1 ) m = 1;
	    if( wm->real_window_width != w || wm->real_window_height != h )
	    {
		sprofile_set_int_value( KEY_SCREENX, wm->real_window_width, NULL );
		sprofile_set_int_value( KEY_SCREENY, wm->real_window_height, NULL );
		save = 1;
	    }
	    if( m != ( ( wm->flags & WIN_INIT_FLAG_MAXIMIZED ) != 0 ) )
	    {
		if( wm->flags & WIN_INIT_FLAG_MAXIMIZED )
		    sprofile_set_int_value( KEY_MAXIMIZED, 1, NULL );
		else
		    sprofile_remove_key( KEY_MAXIMIZED, NULL );
		save = 1;
	    }
	    if( save ) sprofile_save( NULL );
	}
    }
#endif
}
...
static WINDOWPTR get_from_trash( window_manager* wm )
{
    ...
    size_t i = 0;
    for( i = 0; i < smem_get_size( wm->trash ) / sizeof( WINDOWPTR ); i++ )
    {
	if( wm->trash[ i ] )
	{
	    WINDOWPTR win = wm->trash[ i ];
	    wm->trash[ i ] = NULL;
    ...
    window_manager* wm )
{
    ...
    win->click_time = stime_ticks_hires() - stime_ticks_per_second_hires() * 10;
...
static void move_to_trash( WINDOWPTR win, window_manager* wm )
{
	...
	if( !wm->trash )
	{
	    wm->trash = (WINDOWPTR*)smem_znew( sizeof( WINDOWPTR ) * 16 );
	    wm->trash[ 0 ] = win;
	}
	else
	{
	    size_t w = 0;
	    for( w = 0; w < smem_get_size( wm->trash ) / sizeof( WINDOWPTR ); w++ )
	    {
		if( wm->trash[ w ] == NULL ) break;
	    }
	    if( w < smem_get_size( wm->trash ) / sizeof( WINDOWPTR ) ) 
		wm->trash[ w ] = win;
	    else
	    {
		wm->trash = (WINDOWPTR*)smem_resize2( wm->trash, smem_get_size( wm->trash ) + sizeof( WINDOWPTR ) * 16 );
		wm->trash[ w ] = win;
	    }
	}
    }
    ...
    window_manager* wm )
{
	...
	evt->type = (uint16_t)type;
        evt->time = stime_ticks_hires();
        evt->win = win;
        evt->flags = (uint16_t)flags;
        evt->x = (int16_t)x;
        evt->y = (int16_t)y;
        evt->key = (uint16_t)key;
        evt->scancode = (uint16_t)scancode;
        evt->pressure = (uint16_t)pressure;
...
static int check_event( sundog_event* evt, window_manager* wm )
{
    ...
    if( wm->focus_win )
    {
        ...
        if( evt->type == EVT_MOUSEBUTTONDOWN )
        {
    	    if( ( evt->time - wm->focus_win->click_time ) < ( wm->double_click_time * stime_ticks_per_second_hires() ) / 1000 )
	    {
	        //Double click detected:
	        wm->focus_win->flags |= WIN_FLAG_DOUBLECLICK;
	        wm->focus_win->click_time = evt->time - stime_ticks_per_second_hires() * 10; //Reset click time
	    }
	    else
	    {
...
int EVENT_LOOP_END( window_manager* wm )
{
	...
	if( wm->timers_num )
	{
		...
		if( timer->handler )
		{
			...
			{
			    timer->deadline = stime_ticks() + timer->delay;
			}
		    }
		}
	    }
...
void show_status_message( const char* msg, int t, window_manager* wm )
{
	...
	wm->status_timer = add_timer( status_timer, 0, stime_ticks_per_second() * t, wm );
    }
    else 
    {
	reset_timer( wm->status_timer, stime_ticks_per_second() * t, wm );
    }
...
int add_timer( sundog_timer_handler hnd, void* data, ticks_t delay, window_manager* wm )
{
    ...
    wm->timers[ t ].deadline = stime_ticks() + delay;
...
void reset_timer( int timer, window_manager* wm )
{
	...
	if( wm->timers[ i ].id == timer )
	{
	    ...
	    {
		wm->timers[ i ].deadline = stime_ticks() + wm->timers[ i ].delay;
		pack_timers( wm );
	    }
	    break;
	}
...
void reset_timer( int timer, ticks_t new_delay, window_manager* wm )
{
	...
	if( wm->timers[ i ].id == timer )
	{
	    if( wm->timers[ i ].handler )
	    {
		wm->timers[ i ].deadline = stime_ticks() + new_delay;
...
static void reset_decorator( WINDOWPTR dec )
{
    ...
    if( ( flags & DECOR_FLAG_MAXIMIZED ) == 0 && ( flags & DECOR_FLAG_MINIMIZED ) == 0 )
    {
	int x = 0;
	int y = 0;
...
int video_capture_encode( window_manager* wm )
{
    ...
    slog( "Video capture encoding finished (%d). Time: %f sec\n", rv, (float)( stime_ticks() - t ) / (float)stime_ticks_per_second() );
#endif
    return rv;
}
...
void edialog_open( const char* error_str1, const char* error_str2, window_manager* wm )
{
    ...
    char* log = slog_get_latest( error_log_bytes );
...
#ifdef OS_ANDROID
    android_sundog_check_for_permissions( wm->sd, 1 );
#endif
