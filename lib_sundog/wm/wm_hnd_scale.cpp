...
int ui_scale_ok_handler( void* user_data, WINDOWPTR win, window_manager* wm )
{
	...
	if( data->auto_scale )
	{
	    sprofile_remove_key( KEY_PPI, 0 );
	    sprofile_remove_key( KEY_SCALE, 0 );
	    sprofile_remove_key( KEY_FONT_SCALE, 0 );
	}
	else
	{
	    ...
	    sprofile_set_int_value( KEY_PPI, ppi, 0 );
	    sprofile_set_int_value( KEY_SCALE, scale, 0 );
	    sprofile_set_int_value( KEY_FONT_SCALE, fscale, 0 );
	}
	sprofile_save( 0 );
...
int ui_scale_handler( sundog_event* evt, window_manager* wm )
{
	...
	case EVT_AFTERCREATE:
		...
		if( sprofile_get_int_value( KEY_PPI, 0, 0 ) == 0 && sprofile_get_int_value( KEY_SCALE, 0, 0 ) == 0 )
		    data->auto_scale = 1;
		else
		    data->auto_scale = 0;
		ui_scale_reinit( win );
