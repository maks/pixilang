...
void sundog_deinit( ios_sundog_engine* sd )
{
	...
	STIME_WAIT_FOR( sd->eventloop_stop_answer == false, 4000, 10, "Thread stop timeout!" );
    }
...
void sundog_pause( ios_sundog_engine* sd )
{
    ...
    STIME_WAIT_FOR( sd->eventloop_stop_answer == true, 4000, 10, "App pause timeout!" );
}
...
#endif
    sd->eventloop_stop_request = false;
    STIME_WAIT_FOR( sd->eventloop_stop_answer == false, 4000, 10, "App resume timeout!" );
}
...
int ios_sundog_copy( sundog_engine* s, const char* filename, uint32_t flags )
{
	...
	char* new_filename = sfs_make_filename( filename, true );
...
- (sundog_engine*)auInit:(AVAudioFormat*)format //called from AUAudioUnit
{
