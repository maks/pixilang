	...
	while( d->thread_exit_request == 0 && reset_request == 0 )
	{
		...
		if( rv >= 0 && ev && ev->dest.client == snd_seq_client_id( d->handle ) )
		{
			...
			if( data_size > 0 )
			{
			    ...
			    printf( "\n" );*/
			    ticks_hr_t t = (ticks_hr_t)( ev->time.time.tv_nsec / ( 1000000000 / stime_ticks_per_second_hires() ) ) + ev->time.time.tv_sec * stime_ticks_per_second_hires();
			    t += d->time_offset;
...
int device_midi_client_open( sundog_midi_client* c, const char* name )
{
	...
	snd_seq_sync_output_queue( d->handle );
        ...
        d->time_offset = stime_ticks_hires();
	/*
	...
	sthread_create( &d->thread, midi_client_thread, c, 0 );
...
int device_midi_client_get_devices( sundog_midi_client* c, char*** devices, uint32_t flags )
{
    ...
    *devices = 0;
	...
	while( snd_seq_query_next_port( d->handle, pinfo ) >= 0 ) 
	{
		...
		smem_objlist_add( (void***)devices, port_name2, false, rv - 1 );
	    }
	}
    }
...
int device_midi_client_send_event( sundog_midi_client* c, int pnum, sundog_midi_event* evt )
{
	...
	if( s > 0 )
	{
		...
		prev_t = evt_t;
		*/
		int64_t nsec = (int64_t)( evt_t % stime_ticks_per_second_hires() ) * 1000000000 / stime_ticks_per_second_hires();
		t.tv_sec = evt_t / stime_ticks_per_second_hires();
		t.tv_nsec = (uint)nsec;
