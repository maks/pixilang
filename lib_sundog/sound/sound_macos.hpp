    ...
    AudioBufferList* ioData )
{
	...
	else
	{
		...
		for( int i = ( ss->out_frames - 1 ) * ss->out_channels; i >= 0; i-- )
...
int device_sound_init( sundog_sound* ss )
{
    ...
    ss->out_latency = sprofile_get_int_value( KEY_SOUNDBUFFER, g_default_buffer_size, 0 );
    ss->out_latency2 = ss->out_latency;
