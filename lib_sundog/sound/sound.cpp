...
int sundog_sound_callback( sundog_sound* ss, uint32_t flags )
{
	...
	for( int slot_num = 0, slot_bit = 1; slot_num < ss->slot_cnt; slot_num++, slot_bit<<=1 )
	{
	    ...
	    int (*callback)( sundog_sound*, int ) = ( int (*)( sundog_sound*, int ) )slot->callback;
	    slot->out_buf_ptr = 0;
		...
		slot->buffer = ss->slot_buffer;
		    ...
		    slot->frames = size;
	    	    slot->time = ss->out_time;
		    if( slot->out_buf_ptr )
		        slot->time += (int64_t)slot->out_buf_ptr * stime_ticks_per_second_hires() / ss->freq;
	    	    int r = callback( ss, slot_num );
	    	    if( r == 1 ) silence = false;
	    	    if( r )
		    {
...
int sundog_sound_set_defaults( sundog_sound* ss ) //check the initial values (type,freq,channels,flags) and set defaults
{
    if( !ss ) return -1;
    ...
    if( ss->freq <= 0 ) ss->freq = sprofile_get_int_value( KEY_FREQ, 44100, 0 );
...
int sundog_sound_init( sundog_sound* ss, sundog_engine* sd, sound_buffer_type type, int freq, int channels, uint32_t flags )
{
	...
	driver_name = sprofile_get_str_value( KEY_AUDIODRIVER, "", 0 );
        if( driver_name )
	{
...
int sundog_sound_get_free_slot( sundog_sound* ss )
{
    ...
    {
        void* c = sundog_sound_get_slot_callback( ss, slot );
        if( c == NULL ) break;
    }
...
int sundog_sound_capture_start( sundog_sound* ss, const char* filename, uint32_t flags )
{
	...
	ss->out_file_buf_rp = 0;
        ...
        sthread_create( &ss->out_file_thread, sundog_sound_capture_thread, ss, 0 );
...
int sundog_midi_client_open( sundog_midi_client* c, sundog_engine* sd, sundog_sound* ss, const char* name, uint32_t flags )
{
    ...
    flags |= MIDI_CLIENT_FLAG_PORTS_MUTEX; //for midi_notify_callback() that can change the ports (remove/reopen) in bg
					   //But is it really necessary? Additional tests are required.
#endif
    ...
    const char* driver_name = NULL;
    driver_name = sprofile_get_str_value( KEY_MIDIDRIVER, "", 0 );
    if( driver_name )
    {
        for( int i = 0; i < NUMBER_OF_MDRIVERS; i++ )
        {
    ...
    {
	slog( "sundog_midi_client_open() error %d", rv );
