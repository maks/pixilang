    ...
    pix_vm* vm )
{
    ...
    size_t word_end_i = 0;
    if( vm->text_lines == 0 ) vm->text_lines = (pix_vm_text_line*)smem_new( sizeof( pix_vm_text_line ) );
...
check_again:
	if( c32 == 0xA )
	{
	    ...
	    if( lines >= old_size )
		vm->text_lines = (pix_vm_text_line*)smem_resize2( vm->text_lines, ( old_size * 2 ) * sizeof( pix_vm_text_line ) );
