// JPEG encoder
// 2011: modified for SunDog engine
...
#define clear_obj( obj ) { smem_clear_struct( obj ); }
...
#define DCT_MUL( var, c ) ( (int16_t)(var) * (int)(c) )
...
static void je_optimize_huffman_table( int table_num, int table_len, jpeg_encoder* je )
{
	...
	je->m_huff_bits[ table_num ][ i ] = (uint8_t)( num_codes[ i ] );
	...
	je->m_huff_val[ table_num ][ num_used_syms - 1 - i ] = (uint8_t)( pSyms[ i ].m_sym_index - 1 );
}

// JPEG marker generation.
static void je_emit_byte( uint8_t i, jpeg_encoder* je )
{
    je->m_all_stream_writes_succeeded = je->m_all_stream_writes_succeeded && ( sfs_putc( i, je->f ) != -1 );
}

static void je_emit_word( uint i, jpeg_encoder* je )
{
    je_emit_byte( (uint8_t)( i >> 8 ), je ); 
    je_emit_byte( (uint8_t)( i & 0xFF ), je );
}

static void je_emit_marker( int marker, jpeg_encoder* je )
{
    je_emit_byte( 0xFF, je ); 
    je_emit_byte( (uint8_t)marker, je );
}
...
static void je_emit_dqt( jpeg_encoder* je )
{
	...
	je_emit_byte( (uint8_t)i, je );
	for( int j = 0; j < 64; j++ )
	    je_emit_byte( (uint8_t)( je->m_quantization_tables[ i ][ j ] ), je );
    }
}
...
static void je_emit_sof( jpeg_encoder* je )
{
    ...
    {
	je_emit_byte( (uint8_t)( i + 1 ), je ); // component ID
...
// Emit Huffman table.
static void je_emit_dht( uint8_t* bits, uint8_t* val, int index, bool ac_flag, jpeg_encoder* je )
{
    ...
    je_emit_byte( (uint8_t)( index + ( ac_flag << 4 ) ), je );
...
static void je_compute_huffman_table( uint* codes, uint8_t* code_sizes, uint8_t* bits, uint8_t* val, jpeg_encoder* je )
{
    int i, l, last_p, si;
    ...
    smem_clear( codes, sizeof( codes[ 0 ] ) * 256 );
    smem_clear( code_sizes, sizeof( code_sizes[ 0 ] ) * 256 );
    for( p = 0; p < last_p; p++ )
    {
...
static void je_compute_quant_table( int* pDst, int16_t* pSrc, jpeg_encoder* je )
{
    int q;
...
static void je_first_pass_init( jpeg_encoder* je )
{
    ...
    smem_clear( je->m_last_dc_val, 3 * sizeof( je->m_last_dc_val[ 0 ] ) );
    je->m_mcu_y_ofs = 0;
    je->m_pass_num = 1;
}
...
static bool je_jpg_open( int p_x_res, int p_y_res, int src_channels, jpeg_encoder* je )
{
    ...
    if( ( je->m_mcu_lines[ 0 ] = (uint8_t*)smem_new( je->m_image_bpl_mcu * je->m_mcu_y ) ) == NULL ) return 0;
    ...
    {
	smem_copy( je->m_huff_bits[ 0 + 0 ], s_dc_lum_bits, 17 );    smem_copy( je->m_huff_val[ 0 + 0 ], s_dc_lum_val, DC_LUM_CODES );
	smem_copy( je->m_huff_bits[ 2 + 0 ], s_ac_lum_bits, 17 );    smem_copy( je->m_huff_val[ 2 + 0 ], s_ac_lum_val, AC_LUM_CODES );
	smem_copy( je->m_huff_bits[ 0 + 1 ], s_dc_chroma_bits, 17 ); smem_copy( je->m_huff_val[ 0 + 1 ], s_dc_chroma_val, DC_CHROMA_CODES );
	smem_copy( je->m_huff_bits[ 2 + 1 ], s_ac_chroma_bits, 17 ); smem_copy( je->m_huff_val[ 2 + 1 ], s_ac_chroma_val, AC_CHROMA_CODES );
...
static void je_load_quantized_coefficients( int component_num, jpeg_encoder* je )
{
    ...
    int16_t* pDst = je->m_coefficient_array;
    for( int i = 0; i < 64; i++ )
    {
...
static void je_flush_output_buffer( jpeg_encoder* je )
{
	...
	je->m_all_stream_writes_succeeded = je->m_all_stream_writes_succeeded && ( sfs_write( je->m_out_buf, 1, JPGE_OUT_BUF_SIZE - je->m_out_buf_left, je->f ) > 0 );
    je->m_pOut_buf = je->m_out_buf;
    je->m_out_buf_left = JPGE_OUT_BUF_SIZE;
}
...
static void je_put_bits( uint bits, uint len, jpeg_encoder* je )
{
    ...
    {
	uint8_t c;
	JPGE_PUT_BYTE( c = (uint8_t)( ( je->m_bit_buffer >> 16 ) & 0xFF ) );
...
static void je_code_coefficients_pass_one( int component_num, jpeg_encoder* je )
{
    ...
    int16_t* src = je->m_coefficient_array;
...
static void je_code_coefficients_pass_two( int component_num, jpeg_encoder* je )
{
    ...
    int16_t* pSrc = je->m_coefficient_array;
    uint* codes[ 2 ];
...
static bool je_process_end_of_image( jpeg_encoder* je )
{
	...
	if( je->m_mcu_y_ofs < 16 ) // check here just to shut up static analysis
	{
		...
		smem_copy( je->m_mcu_lines[ i ], je->m_mcu_lines[ je->m_mcu_y_ofs - 1 ], je->m_image_bpl_mcu );
	}
...
static void je_load_mcu( const void* pSrc, jpeg_encoder* je )
{
    ...
    uint8_t* pDst = je->m_mcu_lines[ je->m_mcu_y_ofs ]; // OK to write up to m_image_bpl_xlt bytes to pDst
    ...
    {
	memset(
	    je->m_mcu_lines[ je->m_mcu_y_ofs ] + je->m_image_bpl_xlt,
	    pDst[ je->m_image_bpl_xlt - 1 ],
	    je->m_image_x_mcu - je->m_image_x );
    }
    else
    {
	const uint8_t y = pDst[ je->m_image_bpl_xlt - 3 + 0 ], cb = pDst[ je->m_image_bpl_xlt - 3 + 1 ], cr = pDst[ je->m_image_bpl_xlt - 3 + 2 ];
	uint8_t* q = je->m_mcu_lines[ je->m_mcu_y_ofs ] + je->m_image_bpl_xlt;
...
{
    smem_free( je->m_mcu_lines[ 0 ] );
    je_clear( je );
}

bool je_set_params( sfs_file f, int width, int height, int src_channels, je_comp_params* comp_params, jpeg_encoder* je )
{
    je_deinit( je );
    if( f == 0 ) return 0;
    if( width < 1 ) return 0;
